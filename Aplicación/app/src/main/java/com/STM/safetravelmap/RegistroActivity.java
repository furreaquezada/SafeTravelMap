...
public class RegistroActivity extends AppCompatActivity {
    ...
    public void registrar() {
        ...
        if (rut.length() < 3) {
            Toast.makeText(getApplicationContext(), "El RUT debe tener al menos 3 caracteres.", Toast.LENGTH_SHORT).show();
            return;
        }

        int edadComparativa = Integer.parseInt(edad);
        if (edadComparativa < 12 || edadComparativa > 100) {
            Toast.makeText(getApplicationContext(), "La edad debe estar entre 12 y 100 años.", Toast.LENGTH_SHORT).show();
            return;
        }
